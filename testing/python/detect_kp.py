...
import sys
sys.path.insert(0, '/home/ashwinrb/caffe/python')
import os
import cv2 as cv
import numpy as np
import scipy
...
#oriImg is the image after readiang with opencv, infile is the file name, model is after running your model, same is the net, same is the param and multiplier.  Pleft and right are dummy arrays
def find_kp(oriImg, model, net, param, multiplier, p_left, p_right):
    ...
    # print('At scale %d, The CNN took %.2f ms.' % ("0", 1000 * (time.time() - start_time)))
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    try:
                        ...
                        return leftWrist, rightWrist, p_left, p_right
    ...
    # left wrist
    # try:
    #     leftWrist = all_peaks[4]
    # except IndexError:
    #     leftWrist = p_left
    #
    # try:
    #     rightWrist = all_peaks[7]
    # except IndexError:
    #     rightWrist = p_right
    #
    # p_left = leftWrist
    ...
    if(len(all_peaks[16]) != 0 ):
        lear = all_peaks[16]
    else:
        lear =  [0,0]

    if(len(all_peaks[17]) != 0):
        rear = all_peaks[17]
    ...
    #cv.imshow("frame.jpg", to_plot)
    #plt.show()

    #print(p_left)
    #return leftWrist, rightWrist, p_left, p_right, lear, rear
    return all_peaks
